...
class BatteryAssign extends StatefulWidget {
  const BatteryAssign({ Key? key }) : super(key: key);

  @override
  State<BatteryAssign> createState() => _BatteryAssignState();
}

class _BatteryAssignState extends State<BatteryAssign> {
  ...
  _assignBatteries() async {
    ...
    if( _scan_len > 5 && _branch_len > 2 ){ 
      await pr.show();
      var baseur = AdsType.baseurl;
      String _url = "$baseur/v1/battery/operations/assign";
      var token = storage.getItem('token');
            try{
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        elevation: 0.0,
        title: Text('Asign Batteries'.toUpperCase(), style:const TextStyle(fontSize: 14, color: Colors.white70,  ),),
              ...
              children: [
                Column(
                  ...
                  Row(children:const [
                    ...
                    Text('Swap center', style: TextStyle(fontSize: 16),),
                  ],),
                  Row(
                    children: [
                      DropdownButton(
