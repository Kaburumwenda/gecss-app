...
class BatteryCharged extends StatefulWidget {
  const BatteryCharged({ Key? key }) : super(key: key);

  @override
  State<BatteryCharged> createState() => _BatteryChargedState();
}

class _BatteryChargedState extends State<BatteryCharged> {
  ...
  _assignBatteries() async {
    ...
    if( _scan_len > 5 && _branch_len > 2 ){ 
      await pr.show();
      var baseur = AdsType.baseurl;
      String _url = "$baseur/v1/battery/operations/charged";
      var token = storage.getItem('token');
            try{
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        elevation: 0.0,
        title: Text('Battery centers'.toUpperCase(), style:const TextStyle(fontSize: 14, color: Colors.white70,  ),),
              ...
              children: [
                Column(
                  ...
                  Row(children:const [
                    ...
                    Text('Battery centers', style: TextStyle(fontSize: 16),),
                  ],),
                  Row(
                    children: [
                      DropdownButton(
                        hint:const Text('Select Battery center'),
